...
CAR_CLASS_IDX = find_class_idx(detail_model, {"car"})
PLATE_CLASS_IDX = find_class_idx(detail_model, {"license-plate", "license_plate", "plate"})

if CAR_CLASS_IDX is None or PLATE_CLASS_IDX is None:
...
def label_image(image_path, label_dir):
    ...
    # Detect cars
    for r in detail_model.predict(img, conf=CAR_CONF, imgsz=IMGSZ, classes=[CAR_CLASS_IDX], verbose=False):
        for box in r.boxes:
    ...
    for r in detail_model.predict(img, conf=PLATE_CONF, imgsz=IMGSZ, classes=[PLATE_CLASS_IDX], verbose=False):
        for box in r.boxes:
            x1, y1, x2, y2 = map(int, box.xyxy[0].tolist())
            cx, cy, w, h = bbox_to_yolo(x1, y1, x2, y2, img_w, img_h)
